...
constructor TTextManager.Create(ALibPath: string);
begin
  inherited Create;
...
procedure TTextManager.LoadLIBFile(FilePath:string; var AArray:array of string);
var
  LIBFile: file of char; //Declare it a simple file
  {LIBFile:file;}
  NumRead:integer;
  LIBHead:record //This should be whole header storage
    NumEntries,Offset,Etc,OtherStuff:word;
  end;
...
begin
  ...
  Reset(LIBFile{,1}); //1 means chunk size, it can be anything, but 1 is simplier to use
  {//Read data into FileData array starting from 0 and length of 999999bytes,
  //NumRead tells us how many bytes were actualy read
  BlockRead(LIBFile,FileData[0],999999,NumRead);}
  ...
  Reset(LIBFile{,1}); //Reset the file so we can start reading from the start again

  {Reset(LIBFile,1); //Reset the file so we can start reading from the start again
  BlockRead(LIBFile,LIBHead.NumEntries,SizeOf(LIBHead)); //Read whole header at once}
  ...
  begin
    ...
    {BlockRead(LIBFile,w,2); //read 2 bytes into signed smallint}
    {if w=-1 then }
...
function TTextManager.GetTextString(AIndex:integer):string;
begin
  if AIndex < MaxStrings then Result := TextStrings[AIndex];
end;

function TTextManager.GetSetupString(AIndex:integer):string;
begin
