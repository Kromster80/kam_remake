...
begin
 ...
 try
  ...
  try
   DIB_Handle := CreateDIBitmap(DC_Handle, BitmapInfo^, CBM_INIT, Pointer(Cardinal(BitmapInfo) + BitmapInfo.biSize+ BitmapInfo.biClrUsed * 4), PBITMAPINFO(BitmapInfo)^, DIB_RGB_COLORS);
  finally
   DeleteDC(DC_Handle);
  end;
...
begin
 if FAVIState = aviNoFile then Exit;
...
begin
  ...
  AVIFileOpen(FAVI.AFile,PChar(FFilename),OF_READ, nil);
  ...
  begin
   ...
   AVIStreamRead(FAVI.Stream_Audio, 0, AVIStreamTimeToSample(FAVI.Stream_Audio, AVIStreamEndTime(FAVI.Stream_Audio)), nil, 0, @BufferSize, nil);
   GetMem(Buffer,BufferSize);
   ZeroMemory(Buffer,BufferSize);
   AVIStreamRead(FAVI.Stream_Audio, 0, AVIStreamTimeToSample(FAVI.Stream_Audio, AVIStreamEndTime(FAVI.Stream_Audio)), Buffer, BufferSize, nil, nil);
...
procedure TAVI.Play;
begin
 if FAVIState in [aviNoFile,aviPlaying] then Exit;
...
procedure TAVI.Stop;
begin
 if FAVIState in [aviNoFile,aviStopped] then Exit;
