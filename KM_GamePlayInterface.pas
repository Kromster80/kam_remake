...
{Building page}
  Page:=gp_Build;
  KMPanel[Page]:=TKMPanel.Create(0,474,200,400);
  ...
  KMButtonFlat[gb_Road]:=TKMButtonFlat.Create(  8,80,32,32,335);
  fControls.Add(KMButtonFlat[gb_Road]);
  KMButtonFlat[gb_Road].ParentTo(KMPanel[Page]);
  KMButtonFlat[gb_Field]:=TKMButtonFlat.Create( 44,80,32,32,337);
  fControls.Add(KMButtonFlat[gb_Field]);
  KMButtonFlat[gb_Field].ParentTo(KMPanel[Page]);
  KMButtonFlat[gb_Wine]:=TKMButtonFlat.Create( 80,80,32,32,336);
  fControls.Add(KMButtonFlat[gb_Wine]);
  KMButtonFlat[gb_Wine].ParentTo(KMPanel[Page]);
  KMButtonFlat[gb_Cancel]:=TKMButtonFlat.Create(152,80,32,32,340);
  ...
  for i:=0 to 4 do for k:=0 to 4 do begin
    ...
    KMButtonFlat[Button]:=TKMButtonFlat.Create( 8+k*36,120+i*36,32,32,GUIBuildIcons[i*5+k+1]);
...
{Menu page}
  ...
  KMButton[1]:=TKMButton.Create(10,20,180,30,'Save game',fnt_Metal);
  fControls.Add(KMButton[1]);
  KMButton[1].ParentTo(KMPanel[Page]);
  KMButton[2]:=TKMButton.Create(10,60,180,30,'Load game',fnt_Metal);
  fControls.Add(KMButton[2]);
  KMButton[2].ParentTo(KMPanel[Page]);
  KMButton[3]:=TKMButton.Create(10,100,180,30,'Options',fnt_Metal);
  fControls.Add(KMButton[3]);
  KMButton[3].ParentTo(KMPanel[Page]);
  KMButton[4]:=TKMButton.Create(10,180,180,30,'Exit',fnt_Metal);
