...
uses
  ...
  KM_ResourceWares, KM_HouseBarracks, KM_ResourceFonts;
...
begin
  ...
  Panel_Main := TKMPanel.Create(fMyControls, 0, 0, aScreenX, aScreenY);
    ...
    TKMLabel.Create(Panel_Main, TB_PAD, 190, TB_WIDTH, 0, fTextMain[TX_MAPED_PLAYERS], fnt_Outline, taLeft);
  ...
  Panel_Common := TKMPanel.Create(Panel_Main,TB_PAD,255,TB_WIDTH,768);
    ...
    Button_Main[1].Hint := fTextMain[TX_MAPED_TERRAIN];
    Button_Main[2].Hint := fTextMain[TX_MAPED_VILLAGE];
    Button_Main[3].Hint := fTextMain[TX_MAPED_SCRIPTS_VISUAL];
    Button_Main[4].Hint := fTextMain[TX_MAPED_SCRIPTS_GLOBAL];
    Button_Main[5].Hint := fTextMain[TX_MAPED_MENU];
    for I := 1 to 5 do
...
begin
  Panel_Terrain := TKMPanel.Create(Panel_Common, 0, 45, TB_WIDTH, 28);
    ...
    begin
      ...
      Button_Terrain[I].Hint := fTextMain[BtnHint[I]];
      Button_Terrain[I].OnClick := SwitchPage;
    end;

    Panel_Brushes := TKMPanel.Create(Panel_Terrain,0,28,TB_WIDTH,400);
      TKMLabel.Create(Panel_Brushes, 0, PAGE_TITLE_Y, TB_WIDTH, 0, fTextMain[TX_MAPED_TERRAIN_BRUSH], fnt_Outline, taCenter);
      ...
      BrushCircle.Hint := fTextMain[TX_MAPED_TERRAIN_HEIGHTS_CIRCLE];
      BrushCircle.OnClick := Terrain_BrushChange;
      BrushSquare := TKMButtonFlat.Create(Panel_Brushes, 134, 28, 24, 24, 593);
      BrushSquare.Hint := fTextMain[TX_MAPED_TERRAIN_HEIGHTS_SQUARE];
      ...
      BrushRandom := TKMCheckBox.Create(Panel_Brushes, 0, 350, TB_WIDTH, 20, fTextMain[TX_MAPED_TERRAIN_BRUSH_RANDOM], fnt_Metal);
      BrushRandom.OnClick := Terrain_BrushChange;

    Panel_Heights := TKMPanel.Create(Panel_Terrain,0,28,TB_WIDTH,400);
      TKMLabel.Create(Panel_Heights, 0, PAGE_TITLE_Y, TB_WIDTH, 0, fTextMain[TX_MAPED_TERRAIN_HEIGHTS], fnt_Outline, taCenter);
      HeightShapeLabel := TKMLabel.Create(Panel_Heights, 0, 34, TB_WIDTH, 0, fTextMain[TX_MAPED_TERRAIN_HEIGHTS_SHAPE], fnt_Metal, taLeft);
      HeightCircle := TKMButtonFlat.Create(Panel_Heights, 120, 30, 24, 24, 592);
      HeightCircle.Hint := fTextMain[TX_MAPED_TERRAIN_HEIGHTS_CIRCLE];
      HeightCircle.OnClick  := Terrain_HeightChange;
      HeightSquare := TKMButtonFlat.Create(Panel_Heights, 150, 30, 24, 24, 593);
      HeightSquare.Hint := fTextMain[TX_MAPED_TERRAIN_HEIGHTS_SQUARE];
      ...
      HeightSize.Caption  := fTextMain[TX_MAPED_TERRAIN_HEIGHTS_SIZE];
      HeightSize.Hint     := fTextMain[TX_MAPED_TERRAIN_HEIGHTS_SIZE_HINT];
      ...
      HeightSlope.Caption   := fTextMain[TX_MAPED_TERRAIN_HEIGHTS_SLOPE];
      HeightSlope.Hint      := fTextMain[TX_MAPED_TERRAIN_HEIGHTS_SLOPE_HINT];
      ...
      HeightSpeed.Caption   := fTextMain[TX_MAPED_TERRAIN_HEIGHTS_SPEED];
      HeightSpeed.Hint      := fTextMain[TX_MAPED_TERRAIN_HEIGHTS_SPEED_HINT];
      ...
      HeightElevate.Caption     := fTextMain[TX_MAPED_TERRAIN_HEIGHTS_ELEVATE];
      HeightElevate.CapOffsetY  := -12;
      HeightElevate.Hint        := fTextMain[TX_MAPED_TERRAIN_HEIGHTS_ELEVATE_HINT];
      HeightUnequalize          := TKMButtonFlat.Create(Panel_Heights, 0, 255, TB_WIDTH, 20, 0);
      HeightUnequalize.OnClick  := Terrain_HeightChange;
      HeightUnequalize.Caption  := fTextMain[TX_MAPED_TERRAIN_HEIGHTS_UNEQUALIZE];
      HeightUnequalize.CapOffsetY  := -12;
      HeightUnequalize.Hint      := fTextMain[TX_MAPED_TERRAIN_HEIGHTS_UNEQUALIZE_HINT];

    Panel_Tiles := TKMPanel.Create(Panel_Terrain, 0, 28, TB_WIDTH, 400);
      TKMLabel.Create(Panel_Tiles, 0, PAGE_TITLE_Y, TB_WIDTH, 0, fTextMain[TX_MAPED_TERRAIN_HINTS_TILES], fnt_Outline, taCenter);

      TilesMagicWater := TKMButtonFlat.Create(Panel_Tiles, 2, 24, TB_WIDTH - 4, 20, 0);
      TilesMagicWater.Caption := fTextMain[TX_MAPED_TERRAIN_MAGIC_WATER];
      TilesMagicWater.CapOffsetY := -12;
      TilesMagicWater.Hint := fTextMain[TX_MAPED_TERRAIN_MAGIC_WATER_HINT];
      TilesMagicWater.OnClick := Terrain_TilesChange;

      TilesRandom := TKMCheckBox.Create(Panel_Tiles, 0, 60, TB_WIDTH, 20, fTextMain[TX_MAPED_TERRAIN_TILES_RANDOM], fnt_Metal);
      TilesRandom.Checked := True;
      TilesRandom.OnClick := Terrain_TilesChange;
      TilesRandom.Hint := fTextMain[TX_MAPED_TERRAIN_TILES_RANDOM_HINT];
    ...
    Panel_Objects := TKMPanel.Create(Panel_Terrain,0,28,TB_WIDTH,400);
      TKMLabel.Create(Panel_Objects, 0, PAGE_TITLE_Y, TB_WIDTH, 0, fTextMain[TX_MAPED_OBJECTS], fnt_Outline, taCenter);
      ...
      ObjectErase.Hint := fTextMain[TX_MAPED_TERRAIN_OBJECTS_REMOVE];
      ...
      ObjectBlock.Hint := fTextMain[TX_MAPED_TERRAIN_OBJECTS_BLOCK];
    ...
    Panel_Selection := TKMPanel.Create(Panel_Terrain,0,28,TB_WIDTH,400);
      TKMLabel.Create(Panel_Selection, 0, PAGE_TITLE_Y, TB_WIDTH, 0, fTextMain[TX_MAPED_COPY_TITLE], fnt_Outline, taCenter);
        Button_SelectCopy := TKMButton.Create(Panel_Selection, 20, 30, TB_WIDTH - 40, 20, fTextMain[TX_MAPED_COPY], bsGame);
        Button_SelectCopy.Hint := fTextMain[TX_MAPED_COPY_COPY_HINT];
        Button_SelectCopy.OnClick := Terrain_SelectionClick;
        Button_SelectPaste := TKMButton.Create(Panel_Selection, 20, 60, TB_WIDTH - 40, 20, fTextMain[TX_MAPED_PASTE], bsGame);
        Button_SelectPaste.Hint := fTextMain[TX_MAPED_COPY_PASTE_HINT];
        Button_SelectPaste.OnClick := Terrain_SelectionClick;
        Button_SelectPasteApply := TKMButton.Create(Panel_Selection, 20, 90, TB_WIDTH - 40, 20, fTextMain[TX_MAPED_PASTE_APPLY], bsGame);
        Button_SelectPasteApply.Hint := fTextMain[TX_MAPED_COPY_PASTE_HINT];
        Button_SelectPasteApply.OnClick := Terrain_SelectionClick;
        Button_SelectPasteCancel := TKMButton.Create(Panel_Selection, 20, 120, TB_WIDTH - 40, 20, fTextMain[TX_MAPED_PASTE_CANCEL], bsGame);
        Button_SelectPasteCancel.Hint := fTextMain[TX_MAPED_COPY_PASTE_HINT];
        Button_SelectPasteCancel.OnClick := Terrain_SelectionClick;
        Button_SelectFlipH := TKMButton.Create(Panel_Selection, 20, 150, TB_WIDTH - 40, 20, fTextMain[TX_MAPED_COPY_PASTE_HFLIP], bsGame);
        Button_SelectFlipH.Hint := fTextMain[TX_MAPED_COPY_PASTE_HFLIP_HINT];
        Button_SelectFlipH.OnClick := Terrain_SelectionClick;
        Button_SelectFlipH.Hide; //Not implemented yet
        Button_SelectFlipV := TKMButton.Create(Panel_Selection, 20, 180, TB_WIDTH - 40, 20, fTextMain[TX_MAPED_COPY_PASTE_VFLIP], bsGame);
        Button_SelectFlipV.Hint := fTextMain[TX_MAPED_COPY_PASTE_VFLIP_HINT];
        Button_SelectFlipV.OnClick := Terrain_SelectionClick;
        Button_SelectFlipV.Hide; //Not implemented yet
        with TKMLabel.Create(Panel_Selection, 8, 190, TB_WIDTH-16, 80, fTextMain[TX_MAPED_COPY_SELECT_HINT], fnt_Grey, taLeft) do
          AutoWrap := True;
end;
...
begin
  Panel_Town := TKMPanel.Create(Panel_Common, 0, 45, TB_WIDTH, 28);
    ...
    begin
      ...
      Button_Town[VT].Hint := fTextMain[TabHint[VT]];
      Button_Town[VT].OnClick := SwitchPage;
    end;

    //Town placement
    Panel_Build := TKMPanel.Create(Panel_Town,0,28,TB_WIDTH,400);
      TKMLabel.Create(Panel_Build,0,PAGE_TITLE_Y,TB_WIDTH,0,fTextMain[TX_MAPED_ROAD_TITLE],fnt_Outline,taCenter);
      ...
      Button_BuildRoad.Hint     := fTextMain[TX_BUILD_ROAD_HINT];
      Button_BuildField.Hint    := fTextMain[TX_BUILD_FIELD_HINT];
      Button_BuildWine.Hint     := fTextMain[TX_BUILD_WINE_HINT];
      Button_BuildCancel.Hint   := fTextMain[TX_BUILD_CANCEL_HINT];

      TKMLabel.Create(Panel_Build,0,65,TB_WIDTH,0,fTextMain[TX_MAPED_HOUSES_TITLE],fnt_Outline,taCenter);
    ...
    Panel_Units := TKMPanel.Create(Panel_Town,0,28,TB_WIDTH,400);
      ...
      Button_UnitCancel.Hint := fTextMain[TX_BUILD_CANCEL_HINT];
    ...
    Panel_Script := TKMPanel.Create(Panel_Town, 0, 28, TB_WIDTH, 400);
      TKMLabel.Create(Panel_Script, 0, PAGE_TITLE_Y, TB_WIDTH, 0, fTextMain[TX_MAPED_AI_TITLE], fnt_Outline, taCenter);
      CheckBox_AutoBuild := TKMCheckBox.Create(Panel_Script, 0, 30, TB_WIDTH, 20, fTextMain[TX_MAPED_AI_AUTOBUILD], fnt_Metal);
      CheckBox_AutoBuild.OnClick := Town_ScriptChange;
      CheckBox_AutoRepair := TKMCheckBox.Create(Panel_Script, 0, 50, TB_WIDTH, 20, fTextMain[TX_MAPED_AI_AUTOREPAIR], fnt_Metal);
      ...
      TrackBar_SerfsPer10Houses.Caption := fTextMain[TX_MAPED_AI_SERFS_PER_10_HOUSES];
      ...
      TrackBar_WorkerCount.Caption := fTextMain[TX_MAPED_AI_WORKERS];
      TrackBar_WorkerCount.OnChange := Town_ScriptChange;

    //Defence settings
    Panel_Defence := TKMPanel.Create(Panel_Town, 0, 28, TB_WIDTH, 400);
      TKMLabel.Create(Panel_Defence, 0, PAGE_TITLE_Y, TB_WIDTH, 0, fTextMain[TX_MAPED_AI_DEFENSE], fnt_Outline, taCenter);
      Button_DefencePosAdd := TKMButtonFlat.Create(Panel_Defence, 0, 30, 33, 33, 338);
      Button_DefencePosAdd.OnClick := Town_DefenceAddClick;
      Button_DefencePosAdd.Hint    := fTextMain[TX_MAPED_AI_DEFENSE_HINT];

      TKMLabel.Create(Panel_Defence, 0, 65, TB_WIDTH, 0, fTextMain[TX_MAPED_AI_DEFENSE_OPTIONS], fnt_Outline, taCenter);
      CheckBox_AutoDefence := TKMCheckBox.Create(Panel_Defence, 0, 90, TB_WIDTH, 20, fTextMain[TX_MAPED_AI_DEFENSE_AUTO], fnt_Metal);
      CheckBox_AutoDefence.Hint := fTextMain[TX_MAPED_AI_DEFENSE_AUTO_HINT];
      ...
      TrackBar_EquipRateLeather.Caption := fTextMain[TX_MAPED_AI_DEFENSE_EQUIP_LEATHER];
      ...
      TrackBar_EquipRateIron.Caption := fTextMain[TX_MAPED_AI_DEFENSE_EQUIP_IRON];
      ...
      TrackBar_RecruitCount.Caption := fTextMain[TX_MAPED_AI_RECRUITS];
      TrackBar_RecruitCount.Hint := fTextMain[TX_MAPED_AI_RECRUITS_HINT];
      ...
      TrackBar_RecruitDelay.Caption := fTextMain[TX_MAPED_AI_RECRUIT_DELAY];
      TrackBar_RecruitDelay.Hint := fTextMain[TX_MAPED_AI_RECRUIT_DELAY_HINT];
      TrackBar_RecruitDelay.Step := 5;
      TrackBar_RecruitDelay.OnChange := Town_DefenceChange;

      CheckBox_MaxSoldiers := TKMCheckBox.Create(Panel_Defence, 0, 296, TB_WIDTH, 20, fTextMain[TX_MAPED_AI_MAX_SOLDIERS], fnt_Metal);
      CheckBox_MaxSoldiers.Hint := fTextMain[TX_MAPED_AI_MAX_SOLDIERS_ENABLE_HINT];
      ...
      TrackBar_MaxSoldiers.Hint := fTextMain[TX_MAPED_AI_MAX_SOLDIERS_HINT];
      TrackBar_MaxSoldiers.Step := 5;
      TrackBar_MaxSoldiers.OnChange := Town_DefenceChange;

      Button_EditFormations := TKMButton.Create(Panel_Defence, 0, 344, TB_WIDTH, 25, fTextMain[TX_MAPED_AI_FORMATIONS], bsGame);
      Button_EditFormations.OnClick := Formations_Show;

    //Offence settings
    Panel_Offence := TKMPanel.Create(Panel_Town, 0, 28, TB_WIDTH, 400);
      TKMLabel.Create(Panel_Offence, 0, PAGE_TITLE_Y, TB_WIDTH, 0, fTextMain[TX_MAPED_AI_ATTACK], fnt_Outline, taCenter);

      CheckBox_AutoAttack := TKMCheckBox.Create(Panel_Offence, 0, 30, TB_WIDTH, 20, fTextMain[TX_MAPED_AI_ATTACK_AUTO], fnt_Metal);
      ...
      ColumnBox_Attacks.SetColumns(fnt_Outline,
        [fTextMain[TX_MAPED_AI_ATTACK_COL_TYPE],
         fTextMain[TX_MAPED_AI_ATTACK_COL_DELAY],
         fTextMain[TX_MAPED_AI_ATTACK_COL_MEN],
         fTextMain[TX_MAPED_AI_ATTACK_COL_TARGET],
         fTextMain[TX_MAPED_AI_ATTACK_COL_LOC]], [0, 20, 60, 100, 130]);
...
begin
  Panel_Player := TKMPanel.Create(Panel_Common,0,45, TB_WIDTH,28);
    ...
    begin
      ...
      Button_Player[PT].Hint := fTextMain[TabHint[PT]];
      Button_Player[PT].OnClick := SwitchPage;
    end;

    //Goals
    Panel_Goals := TKMPanel.Create(Panel_Player,0,28,TB_WIDTH,400);
      TKMLabel.Create(Panel_Goals, 0, PAGE_TITLE_Y, TB_WIDTH, 0, fTextMain[TX_MAPED_GOALS], fnt_Outline, taCenter);
      ColumnBox_Goals := TKMColumnBox.Create(Panel_Goals, 0, 30, TB_WIDTH, 230, fnt_Game, bsGame);
      ColumnBox_Goals.SetColumns(fnt_Outline,
        [fTextMain[TX_MAPED_GOALS_TYPE],
         fTextMain[TX_MAPED_GOALS_CONDITION],
         fTextMain[TX_MAPED_GOALS_PLAYER],
         fTextMain[TX_MAPED_GOALS_TIME],
         fTextMain[TX_MAPED_GOALS_MESSAGE]], [0, 20, 120, 140, 160]);
    ...
    Panel_Color := TKMPanel.Create(Panel_Player, 0, 28, TB_WIDTH, 400);
      TKMLabel.Create(Panel_Color, 0, PAGE_TITLE_Y, TB_WIDTH, 0, fTextMain[TX_MAPED_PLAYER_COLORS], fnt_Outline, taCenter);
    ...
    Panel_BlockHouse := TKMPanel.Create(Panel_Player, 0, 28, TB_WIDTH, 400);
      TKMLabel.Create(Panel_BlockHouse, 0, PAGE_TITLE_Y, TB_WIDTH, 0, fTextMain[TX_MAPED_BLOCK_HOUSES], fnt_Outline, taCenter);
      for I := 1 to GUI_HOUSE_COUNT do
    ...
    Panel_BlockTrade := TKMPanel.Create(Panel_Player, 0, 28, TB_WIDTH, 400);
      TKMLabel.Create(Panel_BlockTrade, 0, PAGE_TITLE_Y, TB_WIDTH, 0, fTextMain[TX_MAPED_BLOCK_TRADE], fnt_Outline, taCenter);
    ...
    Panel_Markers := TKMPanel.Create(Panel_Player, 0, 28, TB_WIDTH, 400);
      TKMLabel.Create(Panel_Markers, 0, PAGE_TITLE_Y, TB_WIDTH, 0, fTextMain[TX_MAPED_FOG], fnt_Outline, taCenter);
      Button_Reveal         := TKMButtonFlat.Create(Panel_Markers, 0, 30, 33, 33, 394);
      Button_Reveal.Hint    := fTextMain[TX_MAPED_FOG_HINT];
      ...
      CheckBox_RevealAll          := TKMCheckBox.Create(Panel_Markers, 0, 75, 140, 20, fTextMain[TX_MAPED_FOG_ALL], fnt_Metal);
      CheckBox_RevealAll.OnClick  := Player_MarkerClick;
      TKMLabel.Create(Panel_Markers, 0, 100, TB_WIDTH, 0, fTextMain[TX_MAPED_FOG_CENTER], fnt_Outline, taCenter);
      Button_CenterScreen         := TKMButtonFlat.Create(Panel_Markers, 0, 120, 33, 33, 391);
      Button_CenterScreen.Hint    := fTextMain[TX_MAPED_FOG_CENTER_HINT];
      ...
      Button_PlayerCenterScreen.Hint := fTextMain[TX_MAPED_FOG_CENTER_JUMP];
end;
...
begin
  Panel_Mission := TKMPanel.Create(Panel_Common, 0, 45, TB_WIDTH, 28);
    ...
    Panel_Alliances := TKMPanel.Create(Panel_Mission,0,28,TB_WIDTH,400);
      ...
      CheckBox_AlliancesSym := TKMCheckBox.Create(Panel_Alliances, 0, 50+MAX_PLAYERS*25, TB_WIDTH, 20, fTextMain[TX_MAPED_ALLIANCE_SYMMETRIC], fnt_Metal);
    ...
    Panel_PlayerTypes := TKMPanel.Create(Panel_Mission, 0, 28, TB_WIDTH, 400);
      TKMLabel.Create(Panel_PlayerTypes, 0, PAGE_TITLE_Y, TB_WIDTH, 0, fTextMain[TX_MAPED_PLAYERS_TYPE], fnt_Outline, taCenter);
      TKMLabel.Create(Panel_PlayerTypes,  4, 30, 20, 20, '#',       fnt_Grey, taLeft);
      TKMLabel.Create(Panel_PlayerTypes, 24, 30, 60, 20, fTextMain[TX_MAPED_PLAYERS_DEFAULT], fnt_Grey, taLeft);
...
begin
  Panel_Menu := TKMPanel.Create(Panel_Common, 0, 45, TB_WIDTH, 400);
    Button_Menu_Load := TKMButton.Create(Panel_Menu, 0, 20, TB_WIDTH, 30, fTextMain[TX_MAPED_LOAD_TITLE], bsGame);
    Button_Menu_Load.OnClick := SwitchPage;
    Button_Menu_Load.Hint := fTextMain[TX_MAPED_LOAD_TITLE];
    Button_Menu_Save := TKMButton.Create(Panel_Menu, 0, 60, TB_WIDTH, 30, fTextMain[TX_MAPED_SAVE_TITLE], bsGame);
    Button_Menu_Save.OnClick := SwitchPage;
    Button_Menu_Save.Hint := fTextMain[TX_MAPED_SAVE_TITLE];
    Button_Menu_Settings := TKMButton.Create(Panel_Menu, 0, 100, TB_WIDTH, 30, fTextMain[TX_MENU_SETTINGS], bsGame);
    Button_Menu_Settings.Hint := fTextMain[TX_MENU_SETTINGS];
    Button_Menu_Settings.Disable;
    Button_Menu_Quit := TKMButton.Create(Panel_Menu, 0, 180, TB_WIDTH, 30, fTextMain[TX_MENU_QUIT_MAPED], bsGame);
    Button_Menu_Quit.Hint := fTextMain[TX_MENU_QUIT_MAPED];
...
begin
  Panel_Save := TKMPanel.Create(Panel_Common,0,45,TB_WIDTH,400);
    ...
    Radio_Save_MapType.Add(fTextMain[TX_MENU_MAPED_SPMAPS]);
    Radio_Save_MapType.Add(fTextMain[TX_MENU_MAPED_MPMAPS]);
    Radio_Save_MapType.OnChange := Menu_SaveClick;
    TKMLabel.Create(Panel_Save,0,90,TB_WIDTH,20,fTextMain[TX_MAPED_SAVE_TITLE],fnt_Outline,taCenter);
    Edit_SaveName       := TKMEdit.Create(Panel_Save,0,110,TB_WIDTH,20, fnt_Grey);
    Edit_SaveName.AllowedChars := acFileName;
    Label_SaveExists    := TKMLabel.Create(Panel_Save,0,140,TB_WIDTH,0,fTextMain[TX_MAPED_SAVE_EXISTS],fnt_Outline,taCenter);
    CheckBox_SaveExists := TKMCheckBox.Create(Panel_Save,0,160,TB_WIDTH,20,fTextMain[TX_MAPED_SAVE_OVERWRITE], fnt_Metal);
    Button_SaveSave     := TKMButton.Create(Panel_Save,0,180,TB_WIDTH,30,fTextMain[TX_MAPED_SAVE],bsGame);
    Button_SaveCancel   := TKMButton.Create(Panel_Save,0,220,TB_WIDTH,30,fTextMain[TX_MAPED_SAVE_CANCEL],bsGame);
...
begin
  Panel_Load := TKMPanel.Create(Panel_Common,0,45,TB_WIDTH,400);
    TKMLabel.Create(Panel_Load, 0, PAGE_TITLE_Y, TB_WIDTH, 30, fTextMain[TX_MAPED_LOAD_TITLE], fnt_Outline, taLeft);
    ...
    Radio_Load_MapType.Add(fTextMain[TX_MENU_MAPED_SPMAPS]);
    Radio_Load_MapType.Add(fTextMain[TX_MENU_MAPED_MPMAPS]);
    ...
    Button_LoadLoad     := TKMButton.Create(Panel_Load,0,300,TB_WIDTH,30,fTextMain[TX_MAPED_LOAD],bsGame);
    Button_LoadCancel   := TKMButton.Create(Panel_Load,0,335,TB_WIDTH,30,fTextMain[TX_MAPED_LOAD_CANCEL],bsGame);
...
begin
  Panel_Quit := TKMPanel.Create(Panel_Common, 0, 45, TB_WIDTH, 400);
    TKMLabel.Create(Panel_Quit, 0, 40, TB_WIDTH, 60, fTextMain[TX_MAPED_LOAD_UNSAVED], fnt_Outline, taCenter);
    Button_Quit_Yes := TKMButton.Create(Panel_Quit, 0, 100, TB_WIDTH, 30, fTextMain[TX_MENU_QUIT_MAPED], bsGame);
    Button_Quit_No  := TKMButton.Create(Panel_Quit, 0, 140, TB_WIDTH, 30, fTextMain[TX_MENU_DONT_QUIT_MISSION], bsGame);
    Button_Quit_Yes.Hint    := fTextMain[TX_MENU_QUIT_MAPED];
    Button_Quit_No.Hint     := fTextMain[TX_MENU_DONT_QUIT_MISSION];
...
begin
  ...
  Panel_Extra.Hide;
    ...
    Image_ExtraClose.Hint := fTextMain[TX_MSG_CLOSE_HINT];
    ...
    TrackBar_Passability.Caption := fTextMain[TX_MAPED_VIEW_PASSABILITY];
    TrackBar_Passability.Position := 0; //Disabled by default
    TrackBar_Passability.OnChange := Extra_Change;
    Label_Passability := TKMLabel.Create(Panel_Extra, 50, 114, 180, 0, fTextMain[TX_MAPED_PASSABILITY_OFF], fnt_Antiqua, taLeft);

    CheckBox_ShowObjects := TKMCheckBox.Create(Panel_Extra, 250, 70, 180, 20, fTextMain[TX_MAPED_VIEW_OBJECTS], fnt_Antiqua);
    CheckBox_ShowObjects.Checked := True; //Enabled by default
    CheckBox_ShowObjects.OnClick := Extra_Change;
    CheckBox_ShowHouses := TKMCheckBox.Create(Panel_Extra, 250, 90, 180, 20, fTextMain[TX_MAPED_VIEW_HOUSES], fnt_Antiqua);
    CheckBox_ShowHouses.Checked := True; //Enabled by default
    CheckBox_ShowHouses.OnClick := Extra_Change;
    CheckBox_ShowUnits := TKMCheckBox.Create(Panel_Extra, 250, 110, 180, 20, fTextMain[TX_MAPED_VIEW_UNITS], fnt_Antiqua);
    CheckBox_ShowUnits.Checked := True; //Enabled by default
    CheckBox_ShowUnits.OnClick := Extra_Change;
    CheckBox_ShowDeposits := TKMCheckBox.Create(Panel_Extra, 250, 130, 180, 20, fTextMain[TX_MAPED_VIEW_DEPOSISTS], fnt_Antiqua);
    ...
    Dropbox_PlayerFOW.Hint := fTextMain[TX_REPLAY_PLAYER_PERSPECTIVE];
...
begin
  ...
  Panel_Message.Hide;
    ...
    Image_MessageClose.Hint := fTextMain[TX_MSG_CLOSE_HINT];
...
begin
  ...
  Panel_Attack.Hide;
    ...
    TKMLabel.Create(Panel_Attack, SIZE_X div 2, 10, fTextMain[TX_MAPED_AI_ATTACK_INFO], fnt_Outline, taCenter);

    TKMLabel.Create(Panel_Attack, 20, 40, fTextMain[TX_MAPED_AI_ATTACK_COL_TYPE], fnt_Metal, taLeft);
    Radio_AttackType := TKMRadioGroup.Create(Panel_Attack, 20, 60, 80, 40, fnt_Metal);
    Radio_AttackType.Add(fTextMain[TX_MAPED_AI_ATTACK_TYPE_ONCE]);
    Radio_AttackType.Add(fTextMain[TX_MAPED_AI_ATTACK_TYPE_REP]);
    Radio_AttackType.OnChange := Attack_Change;

    TKMLabel.Create(Panel_Attack, 130, 40, fTextMain[TX_MAPED_AI_ATTACK_DELAY], fnt_Metal, taLeft);
    NumEdit_AttackDelay := TKMNumericEdit.Create(Panel_Attack, 130, 60, 0, High(SmallInt));
    NumEdit_AttackDelay.OnChange := Attack_Change;

    TKMLabel.Create(Panel_Attack, 240, 40, fTextMain[TX_MAPED_AI_ATTACK_COL_MEN], fnt_Metal, taLeft);
    NumEdit_AttackMen := TKMNumericEdit.Create(Panel_Attack, 240, 60, 0, 1000);
    NumEdit_AttackMen.OnChange := Attack_Change;

    TKMLabel.Create(Panel_Attack, 340, 40, fTextMain[TX_MAPED_AI_ATTACK_COUNT], fnt_Metal, taLeft);
    for GT := Low(TGroupType) to High(TGroupType) do
    begin
      TKMLabel.Create(Panel_Attack, 425, 60 + Byte(GT) * 20, 0, 0, fTextMain[GROUP_TEXT[GT]], fnt_Metal, taLeft);
    ...
    CheckBox_AttackTakeAll := TKMCheckBox.Create(Panel_Attack, 340, 145, 160, 20, fTextMain[TX_MAPED_AI_ATTACK_TAKE_ALL], fnt_Metal);
    CheckBox_AttackTakeAll.OnClick := Attack_Change;

    //Second row

    TKMLabel.Create(Panel_Attack, 20, 170, fTextMain[TX_MAPED_AI_ATTACK_COL_TARGET], fnt_Metal, taLeft);
    Radio_AttackTarget := TKMRadioGroup.Create(Panel_Attack, 20, 190, 160, 80, fnt_Metal);
    Radio_AttackTarget.Add(fTextMain[TX_MAPED_AI_TARGET_CLOSEST]);
    Radio_AttackTarget.Add(fTextMain[TX_MAPED_AI_TARGET_HOUSE1]);
    Radio_AttackTarget.Add(fTextMain[TX_MAPED_AI_TARGET_HOUSE2]);
    Radio_AttackTarget.Add(fTextMain[TX_MAPED_AI_TARGET_CUSTOM]);
    Radio_AttackTarget.OnChange := Attack_Change;

    TKMLabel.Create(Panel_Attack, 200, 170, fTextMain[TX_MAPED_AI_TARGET_POS], fnt_Metal, taLeft);
    ...
    Button_AttackOk := TKMButton.Create(Panel_Attack, SIZE_X-20-320-10, SIZE_Y - 50, 160, 30, fTextMain[TX_MAPED_OK], bsMenu);
    Button_AttackOk.OnClick := Attack_Close;
    Button_AttackCancel := TKMButton.Create(Panel_Attack, SIZE_X-20-160, SIZE_Y - 50, 160, 30, fTextMain[TX_MAPED_CANCEL], bsMenu);
...
begin
  ...
  Panel_Formations.Hide;
    ...
    TKMLabel.Create(Panel_Formations, SIZE_X div 2, 10, fTextMain[TX_MAPED_AI_FORMATIONS_TITLE], fnt_Outline, taCenter);

    Image_FormationsFlag := TKMImage.Create(Panel_Formations, 10, 10, 0, 0, 30, rxGuiMain);

    TKMLabel.Create(Panel_Formations, 20, 70, 80, 0, fTextMain[TX_MAPED_AI_FORMATIONS_COUNT], fnt_Metal, taLeft);
    TKMLabel.Create(Panel_Formations, 20, 95, 80, 0, fTextMain[TX_MAPED_AI_FORMATIONS_COLUMNS], fnt_Metal, taLeft);

    for GT := Low(TGroupType) to High(TGroupType) do
    begin
      TKMLabel.Create(Panel_Formations, 130 + Byte(GT) * 110 + 32, 50, 0, 0, fTextMain[T[GT]], fnt_Metal, taCenter);
    ...
    Button_Formations_Ok := TKMButton.Create(Panel_Formations, SIZE_X-20-320-10, 150, 160, 30, fTextMain[TX_MAPED_OK], bsMenu);
    Button_Formations_Ok.OnClick := Formations_Close;
    Button_Formations_Cancel := TKMButton.Create(Panel_Formations, SIZE_X-20-160, 150, 160, 30, fTextMain[TX_MAPED_CANCEL], bsMenu);
...
begin
  ...
  Panel_Goal.Hide;
    ...
    TKMLabel.Create(Panel_Goal, SIZE_X div 2, 10, fTextMain[TX_MAPED_GOALS_TITLE], fnt_Outline, taCenter);

    Image_FormationsFlag := TKMImage.Create(Panel_Goal, 10, 10, 0, 0, 30, rxGuiMain);

    TKMLabel.Create(Panel_Goal, 20, 40, 100, 0, fTextMain[TX_MAPED_GOALS_TYPE], fnt_Metal, taLeft);
    Radio_GoalType := TKMRadioGroup.Create(Panel_Goal, 20, 60, 100, 60, fnt_Metal);
    Radio_GoalType.Add(fTextMain[TX_MAPED_GOALS_TYPE_NONE]);
    Radio_GoalType.Add(fTextMain[TX_MAPED_GOALS_TYPE_VICTORY]);
    Radio_GoalType.Add(fTextMain[TX_MAPED_GOALS_TYPE_SURVIVE]);
    Radio_GoalType.OnChange := Goal_Change;

    TKMLabel.Create(Panel_Goal, 140, 40, 180, 0, fTextMain[TX_MAPED_GOALS_CONDITION], fnt_Metal, taLeft);
    Radio_GoalCondition := TKMRadioGroup.Create(Panel_Goal, 140, 60, 180, 180, fnt_Metal);
    Radio_GoalCondition.Add(fTextMain[TX_MAPED_GOALS_CONDITION_NONE], False);
    Radio_GoalCondition.Add(fTextMain[TX_MAPED_GOALS_CONDITION_TUTORIAL], False);
    Radio_GoalCondition.Add(fTextMain[TX_MAPED_GOALS_CONDITION_TIME], False);
    Radio_GoalCondition.Add(fTextMain[TX_MAPED_GOALS_CONDITION_BUILDS]);
    Radio_GoalCondition.Add(fTextMain[TX_MAPED_GOALS_CONDITION_TROOPS]);
    Radio_GoalCondition.Add(fTextMain[TX_MAPED_GOALS_CONDITION_UNKNOWN], False);
    Radio_GoalCondition.Add(fTextMain[TX_MAPED_GOALS_CONDITION_ASSETS]);
    Radio_GoalCondition.Add(fTextMain[TX_MAPED_GOALS_CONDITION_SERFS]);
    Radio_GoalCondition.Add(fTextMain[TX_MAPED_GOALS_CONDITION_ECONOMY]);
    Radio_GoalCondition.OnChange := Goal_Change;

    TKMLabel.Create(Panel_Goal, 330, 40, fTextMain[TX_MAPED_GOALS_PLAYER], fnt_Metal, taLeft);
    NumEdit_GoalPlayer := TKMNumericEdit.Create(Panel_Goal, 330, 60, 1, MAX_PLAYERS);
    NumEdit_GoalPlayer.OnChange := Goal_Change;

    TKMLabel.Create(Panel_Goal, 480, 40, fTextMain[TX_MAPED_GOALS_TIME], fnt_Metal, taLeft);
    ...
    TKMLabel.Create(Panel_Goal, 480, 90, fTextMain[TX_MAPED_GOALS_MESSAGE], fnt_Metal, taLeft);
    ...
    Button_GoalOk := TKMButton.Create(Panel_Goal, SIZE_X-20-320-10, SIZE_Y - 50, 160, 30, fTextMain[TX_MAPED_OK], bsMenu);
    Button_GoalOk.OnClick := Goal_Close;
    Button_GoalCancel := TKMButton.Create(Panel_Goal, SIZE_X-20-160, SIZE_Y - 50, 160, 30, fTextMain[TX_MAPED_CANCEL], bsMenu);
...
begin
  Panel_Unit := TKMPanel.Create(Panel_Common, 0, 45, TB_WIDTH, 400);
    ...
    Label_UnitCondition   := TKMLabel.Create(Panel_Unit,65,40,116,0,fTextMain[TX_UNIT_CONDITION],fnt_Grey,taCenter);
    ...
    TKMLabel.Create(Panel_Army, 0, 140, TB_WIDTH, 0, fTextMain[TX_MAPED_GROUP_ORDER], fnt_Outline, taLeft);
    DropBox_ArmyOrder   := TKMDropList.Create(Panel_Army, 0, 160, TB_WIDTH, 20, fnt_Metal, '', bsGame);
    DropBox_ArmyOrder.Add(fTextMain[TX_MAPED_GROUP_ORDER_NONE]);
    DropBox_ArmyOrder.Add(fTextMain[TX_MAPED_GROUP_ORDER_WALK]);
    DropBox_ArmyOrder.Add(fTextMain[TX_MAPED_GROUP_ORDER_ATTACK]);
    ...
    TKMLabel.Create(Panel_Army, 110, 185, fTextMain[TX_MAPED_GROUP_ORDER_DIRECTION], fnt_Grey, taLeft);
...
begin
  Panel_House := TKMPanel.Create(Panel_Common, 0, 45, TB_WIDTH, 400);
    ...
    Label_HouseHealth := TKMLabel.Create(Panel_House, 100, 41, 60, 20, fTextMain[TX_HOUSE_CONDITION], fnt_Mini, taCenter);
    ...
    Label_House_Supply := TKMLabel.Create(Panel_House, 0, 85, TB_WIDTH, 0, fTextMain[TX_HOUSE_SUPPLIES], fnt_Grey, taCenter);
...
begin
  ...
  Image_MarkerPic := TKMImage.Create(Panel_Marker, 0, 10, 32, 32, 338);
    ...
    Panel_MarkerReveal := TKMPanel.Create(Panel_Marker, 0, 46, TB_WIDTH, 400);
      ...
      TrackBar_RevealSize.Caption := fTextMain[TX_MAPED_FOG_RADIUS];
      ...
      Button_RevealDelete.Hint := fTextMain[TX_MAPED_DELETE_REVEALER_HINT];
      Button_RevealDelete.OnClick := Marker_Change;
      Button_RevealClose := TKMButton.Create(Panel_MarkerReveal, TB_WIDTH-100, 55, 100, 25, fTextMain[TX_MAPED_CLOSE], bsGame);
      Button_RevealClose.Hint := fTextMain[TX_MAPED_CLOSE_REVEALER_HINT];
    ...
    Panel_MarkerDefence := TKMPanel.Create(Panel_Marker, 0, 46, TB_WIDTH, 400);
      ...
      DropList_DefenceGroup.Add(fTextMain[TX_MAPED_AI_ATTACK_TYPE_MELEE]);
      DropList_DefenceGroup.Add(fTextMain[TX_MAPED_AI_ATTACK_TYPE_ANTIHORSE]);
      DropList_DefenceGroup.Add(fTextMain[TX_MAPED_AI_ATTACK_TYPE_RANGED]);
      DropList_DefenceGroup.Add(fTextMain[TX_MAPED_AI_ATTACK_TYPE_MOUNTED]);
      ...
      DropList_DefenceType.Add(fTextMain[TX_MAPED_AI_DEFENCE_FRONTLINE]);
      DropList_DefenceType.Add(fTextMain[TX_MAPED_AI_DEFENCE_BACKLINE]);
      ...
      TrackBar_DefenceRad.Caption := fTextMain[TX_MAPED_AI_DEFENCE_RADIUS];
      ...
      Button_DefenceDelete.Hint := fTextMain[TX_MAPED_AI_DEFENCE_DELETE_HINT];
      Button_DefenceDelete.OnClick := Marker_Change;
      Button_DefenceClose := TKMButton.Create(Panel_MarkerDefence, TB_WIDTH-100, 165, 100, 25, fTextMain[TX_MAPED_CLOSE], bsGame);
      Button_DefenceClose.Hint := fTextMain[TX_MAPED_AI_DEFENCE_CLOSE_HINT];
...
begin
  ...
  if TrackBar_Passability.Position <> 0 then
    ...
    Label_Passability.Caption := fTextMain[TX_MAPED_PASSABILITY_OFF];
...
begin
  ...
  case aMarker.MarkerType of
    ...
    mtRevealFOW:  begin
                    Label_MarkerType.Caption := fTextMain[TX_MAPED_FOG];
