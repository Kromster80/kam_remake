...
//|===================| <- constant name length
  ...
  GAME_REVISION         = 'r6547';       //Should be updated for every release (each time save format is changed)
  {$IFDEF USESECUREAUTH}
    GAME_VERSION_POSTFIX  = '';
  {$ELSE}
    GAME_VERSION_POSTFIX  = ' (UNSECURE!)';
  {$ENDIF}
  GAME_VERSION_PREFIX   = 'Next Demo Release Candidate 2'; //Game version string displayed in menu corner
...
var
  ...
  THREADED_FONT_LOAD    :Boolean = True; //Load each font in separate thread to improve load times
