...
//|===================| <- constant name length
  ...
  GAME_REVISION         = 'r6618';       //Should be updated for every release (each time save format is changed)
  {$IFDEF USESECUREAUTH}
    GAME_VERSION_POSTFIX  = '';
  {$ELSE}
    GAME_VERSION_POSTFIX  = ' (UNSECURE!)';
  {$ENDIF}
  GAME_VERSION_PREFIX   = 'Next Demo Release Candidate 3'; //Game version string displayed in menu corner
