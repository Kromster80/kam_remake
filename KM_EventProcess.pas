...
begin
  ...
  case fAction.Action of
    eaDelayedMessage: fOwner.AddEvent(MakeTrigger(etTime, -1, [fGameG.GameTickCount + Cardinal(fAction.Params[0])]), MakeAction(eaShowMessage, fAction.Player, [fAction.Params[1]]));
    eaShowMessage:    if MyPlayer.PlayerIndex = fAction.Player then
                        fGameG.ShowMessage(mkText, fTextLibrary.GetMissionString(fAction.Params[0]), KMPoint(0,0));
    eaVictory:        fGameG.PlayerVictory(fAction.Player);
  end;
end;
