...
if (st[k]=#9)or(st[k]=#10)or(st[k]=#13) then st[k]:=' ';
inc(i);
if (k>1)and(((st[k-1]=#32)and(st[k]=#32))or((st[k-1]='!')and(st[k]='!'))) then else
inc(k);
until(i>=FileSize);
...
begin
Form1.Memo2.Clear;
for Row:=1 to CMDCount do begin
...
if Cmd[Row].Command='!SET_FIELD' then begin
Mission.AddRoad(Cmd[Row].Numer[1]+1,Cmd[Row].Numer[2]+1,Mission.ActivePlayer,gpF);
end else
if Cmd[Row].Command='!SET_WINEFIELD' then begin
