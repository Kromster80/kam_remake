...
var i,k:integer;
begin
for i:=0 to 3 do for k:=0 to 3 do begin
...
begin
ActivePlayer:=1;
for i:=1 to 8 do Player[i]:=TPlayer.Create;
end;
...
if (Roads[k,i]<>gpN)and(Owner[k,i]=_Owner) then begin
if Roads[k,i]=gpR then Result:=Result+'!SET_STREET '+inttostr(k-1)+' '+inttostr(i-1)+' ';
if Roads[k,i]=gpF then Result:=Result+'!SET_FIELD '+inttostr(k-1)+' '+inttostr(i-1)+' ';
if Roads[k,i]=gpW then Result:=Result+'!SET_WINEFIELD '+inttostr(k-1)+' '+inttostr(i-1)+' ';
inc(Num);
if Num mod 4 = 0  then Result:=Result+eol;
end;
end;
