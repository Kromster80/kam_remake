...
//==============================================
if BrushMode=bmRelief then begin
Rad:=Form1.ElevSize.Position;
x1:=max(MapXn-Rad,1); x2:=min(MapXn+Rad,Map.X);
y1:=max(MapYn-Rad,1); y2:=min(MapYn+Rad,Map.Y-1);
glbegin (GL_POINTS);
for ii:=y1 to y2 do for kk:=x1 to x2 do begin
if Form1.RG_Relief.ItemIndex=1 then Tmp:=1-max(abs(ii-MapYn),abs(kk-MapXn))/Rad; //pyramid falloff
if Form1.RG_Relief.ItemIndex=0 then Tmp:=1-sqrt(sqr(ii-MapYn)+sqr(kk-MapXn))/Rad;//circular falloff
case LandBrush of
...
if Index=61 then begin //Object 42 is an invisible wall
  ...
  RenderSprite(Tree[ID], pX+ShiftX, pY+ShiftY, 128/40, 128/40);
  if Index=60 then begin
    RenderSprite(Tree[ID], pX+ShiftX+0.65, pY+ShiftY, 128/40, 128/40);
    RenderSprite(Tree[ID], pX+ShiftX, pY+ShiftY+0.65, 128/40, 128/40);
    RenderSprite(Tree[ID], pX+ShiftX+0.65, pY+ShiftY+0.65, 128/40, 128/40);
  end;
...
if Stage=1 then
  ...
  RenderSprite(House[Index], pX+ShiftX, pY+ShiftY, 256/40, 256/40);
