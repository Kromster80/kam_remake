...
if fileexists(text+'data'+ DirectorySeparator + 'gfx' + DirectorySeparator +'pal0.bbm') then begin
assignfile(f,text+text+'data'+ DirectorySeparator + 'gfx' + DirectorySeparator +'pal0.bbm'); reset(f,1);
blockread(f,c,48);
blockread(f,Pal0,768); //256*3
closefile(f);
end else begin
ShowMessage('Unable to locate ' + text+'data'+ DirectorySeparator + 'gfx' + DirectorySeparator +'pal0.bbm'+ ' file');
...
FormLoading.Bar1.StepIt; FormLoading.Refresh;
if fileexists(text+'data'+ DirectorySeparator + 'gfx'+ DirectorySeparator + 'res'+ DirectorySeparator + 'trees.rx') then begin
assignfile(f,text+'data'+ DirectorySeparator + 'gfx'+ DirectorySeparator + 'res'+ DirectorySeparator + 'trees.rx'); reset(f,1);
blockread(f,TreeQty,4);
blockread(f,TreePal,TreeQty);
for ii:=1 to TreeQty do
if TreePal[ii]=1 then begin //entry used
...
ShowMessage('Unable to locate '+text+'data'+ DirectorySeparator + 'gfx'+ DirectorySeparator + 'res'+ DirectorySeparator + 'trees.rx'+' file');
...
FormLoading.Bar1.StepIt;  FormLoading.Refresh;
if fileexists(text+'data' + DirectorySeparator + 'defines' + DirectorySeparator + 'mapelem.dat') then begin
assignfile(f,text+'data' + DirectorySeparator + 'defines' + DirectorySeparator + 'mapelem.dat'); reset(f,1);
//for ii:=1 to 254 do
blockread(f,MapElem[1],MapElemQty*99); //256*3
closefile(f);
end else begin
ShowMessage('Unable to locate '+text+'data' + DirectorySeparator + 'defines' + DirectorySeparator + 'mapelem.dat'+' file');
...
FormLoading.Bar1.StepIt;  FormLoading.Refresh;
rw:=0; row:=1; col:=1; //row/col in ObjPallete
Form1.ObjPallete.ColCount:=length(ObjPalleteTable[1])-1;
Form1.ObjPallete.DefaultColWidth:= (Form1.ObjPallete.Width-5) div (Form1.ObjPallete.ColCount); //(Area / count) - 1 for mortar
ColWidth:=Form1.ObjPallete.DefaultColWidth;
for ii:=1 to length(ObjIndex) do begin
...
sy:=TreeSize[id,2]; if sy>128 then sy:=128;
sx:=TreeSize[id,1]; if sx>128 then sx:=128;
...
inc(rw,w*(ColWidth));
if (rw>=Form1.ObjPallete.Width-5)or(col+w>Form1.ObjPallete.ColCount)or(col>length(ObjPalleteTable[1])-1) then
  begin
  inc(row); col:=1; rw:=0;
  end;
...
for x:=col+1 to Math.min(Form1.ObjPallete.ColCount,length(ObjPalleteTable[1])-1) do ObjPalleteTable[row,x]:=ii;
inc(col,w);
...
FormLoading.Bar1.StepIt; FormLoading.Refresh;
if fileexists(text+'data' + DirectorySeparator + 'gfx' + DirectorySeparator + 'res' + DirectorySeparator + 'houses.rx') then begin
assignfile(f,text+'data' + DirectorySeparator + 'gfx' + DirectorySeparator + 'res' + DirectorySeparator + 'houses.rx'); reset(f,1);
...
ShowMessage('Unable to locate '+text+'data' + DirectorySeparator + 'gfx' + DirectorySeparator + 'res' + DirectorySeparator + 'houses.rx'+' file');
...
mx:=TreeSize[id,1];
my:=TreeSize[id,2];
if mx>128 then mx:=128;
if my>128 then my:=128;

TD:=AllocMem(128*128*4); //GetMem+FillChar(0)
for i:=0 to 127 do for k:=0 to 127 do
